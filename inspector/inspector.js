document.addEventListener("DOMContentLoaded", async () => {
  ...
  btnMassDelete.addEventListener("click", confirmDelete);
  ...
  async function loadData() {
    ...
    if (!sessionCache)
      throw new Error(
        "Unable to get Salesforce session. Please check if you are logged in.",
      );
    ...
    try {
      ...
      if (entityResponse.ok) {
        ...
        console.warn(
          `Could not fetch SObject DurableId. Status: ${entityResponse.status}`,
        );
      }
    } catch (e) {
    ...
    if (sobjectDurableId) {
      ...
      try {
        const fieldDefResponse = await fetch(fieldDefinitionApiUrl, {
          headers,
        });
        if (fieldDefResponse.ok) {
          ...
          console.warn(
            `Could not fetch Field Definition IDs. Status: ${fieldDefResponse.status}`,
          );
        }
      } catch (e) {
    ...
    const [recordResponse, describeResponse] = await Promise.all([
      fetch(recordApiUrl, { headers }),
      fetch(describeApiUrl, { headers }),
    ]);
    if (!recordResponse.ok || !describeResponse.ok)
      throw new Error(
        `Falha na API. Status do Registro: ${recordResponse.status}. Status do Describe: ${describeResponse.status}`,
      );

    const [recordData, describeData] = await Promise.all([
      recordResponse.json(),
      describeResponse.json(),
    ]);
  ...
  function displayRecordData(data, metadataMap) {
    ...
    for (const fieldApiName of sortedFields) {
      ...
      const metadata = metadataMap.get(fieldApiName) || {
        name: fieldApiName,
        label: fieldApiName,
        type: "N/A",
        updateable: false,
      ...
      if (sobjectName && fieldDurableId) {
        ...
        const baseHost = sfHost.startsWith("http")
          ? sfHost
          : `https://${sfHost}`;
  ...
  function confirmDelete() {
    ...
    overlay.innerHTML = `
            <div class="confirm-dialog">
                <h3>Delete Record</h3>
                <p>Are you sure you want to delete this record?<br><strong>${sobjectName}: ${recordId}</strong><br>This action cannot be undone.</p>
                <div class="confirm-actions">
                    <button class="action-btn btn-cancel" id="confirm-no">Cancel</button>
                    <button class="action-btn btn-delete" id="confirm-yes">Delete</button>
                </div>
            </div>
        `;
    ...
    overlay
      ...
      .addEventListener("click", async () => {
        overlay.querySelector("#confirm-yes").disabled = true;
        overlay.querySelector("#confirm-yes").textContent = "Deleting...";
        await handleDelete();
        overlay.remove();
      });
  }
  ...
  async function handleDelete() {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData[0]?.message || "Unknown error deleting record.",
        );
      }
  ...
  async function getSessionFromBackground() {
    return new Promise((resolve) => {
      chrome.runtime.sendMessage(
        { message: "getSfHost", url: sfHost },
        (sfHostParsed) => {
          chrome.runtime.sendMessage(
            { message: "getSession", sfHost: sfHostParsed },
            (session) => {
              resolve(session);
            },
          );
